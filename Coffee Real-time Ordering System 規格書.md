...
每次功能、錯誤修正、API 契約、UI 流程、環境變數、部署流程、測試契約或營運假設有變更時，必須在同一次交付中同步更新相關文件。至少需檢查 `README.md`、`DEVELOPMENT_PROGRESS.md`、本規格書、後端 Swagger/JSDoc route comments、`AGENTS.md`，以及本機代理指南（如 `CLAUDE.md`）。若歷史分工或長版參考脈絡改變，也需更新 `docs/archive/` 內相關文件。若確認不需更新文件，交付說明需明確註記。Markdown 文件需依 `.editorconfig` 使用 UTF-8 with BOM，避免 Windows 工具讀取繁體中文時發生亂碼。
...
* User 只能查看自己的會員點餐紀錄。
* User 登入後進入「訂單追蹤」頁，需顯示自己的會員訂單查詢碼、取餐手機號碼與目前狀態，並可帶入查詢表單。
...
* 訪客在同一裝置重新整理訂單追蹤頁（無 URL query params）時，系統會自動從 localStorage 還原最近一筆含有 `guestToken` 的 session 並自動查詢，無需重新輸入查詢碼；`guestToken` 的存在即代表該裝置建立了此訂單，可安全還原。會員另有 `loadMemberTrackingOrders` 處理，不受此規則影響。
...
* `guestInfo`：訪客訂單 required；會員訂單可保存取餐手機，用於訂單追蹤與取餐核對
* `orderLookupCode`：顧客可見的訂單查詢碼，訪客、會員一般訂單與兌換訂單皆需產生；需 unique
...
* 訂單追蹤頁（`/orders/guest`）— 訪客輸入查詢碼與手機；重新整理時自動還原最近一筆含 guestToken 的 session（同裝置免重新輸入）；User 登入後顯示自己的會員訂單查詢碼、手機與目前狀態，且無既有查詢條件時需自動載入最近一筆進行中訂單；左右雙欄、狀態步驟條
...
| BR-019 | User 點數不足 3 點時不可建立兌換訂單。 |
| BR-020 | 會員一般訂單與兌換訂單也需產生 `orderLookupCode`；前端不可顯示 Mongo 內部 `_id` 作為顧客訂單號。 |
| BR-021 | User 登入後，訂單追蹤頁需列出自己的會員訂單查詢碼、取餐手機與目前狀態。 |
