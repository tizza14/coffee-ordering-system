...
function getActor(req: Request) {
  ...
  const guestToken =
    typeof req.headers['x-guest-token'] === 'string' ? req.headers['x-guest-token'] : undefined;

  if (authHeader?.startsWith('Bearer ')) {
    try {
      const payload = jwt.verify(authHeader.split(' ')[1], env.jwtSecret) as TokenPayload;
      return {
...
export const requestLinePay = asyncHandler(async (req: Request, res: Response) => {
  const result = await paymentService.requestLinePay(req.body.orderId, getActor(req));
...
export const cancelLinePay = asyncHandler(async (req: Request, res: Response) => {
