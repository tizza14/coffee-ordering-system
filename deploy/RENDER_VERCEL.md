...
- 手機寬度開啟前端首頁，Header 應只顯示品牌與選單按鈕；點選單後從右側顯示導覽。
- 訪客模式不顯示「點餐紀錄」入口，只能使用「訂單追蹤」查詢自己的訪客訂單。
- User 登入後「訂單追蹤」應顯示自己的會員訂單查詢碼、取餐手機與目前狀態；「點餐紀錄」只顯示自己的會員訂單。
- Staff/Admin 可查看完整點餐紀錄。
- 商品頁「前往結帳」與結帳頁「前往付款」按鈕在手機為滿版寬度，平板/桌機為固定寬度，切換會員點餐/訪客點餐時尺寸不跳動。
- 訪客或會員結帳後，付款回導網址不可出現 `localhost`。
- 畫面不得把 Mongo 內部 `order.id` 當作訂單號顯示；應顯示 `orderLookupCode`，沒有查詢碼時顯示中性文案。
